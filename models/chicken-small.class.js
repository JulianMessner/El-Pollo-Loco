class ChickenSmall extends MoveableObject {
  height = 60;
  width = 60;
  chicken_sound = allSounds[5];

  CHICKEN_SMALL_WALKING = [
    "img/3_enemies_chicken/chicken_small/1_walk/1_w.png",
    "img/3_enemies_chicken/chicken_small/1_walk/2_w.png",
    "img/3_enemies_chicken/chicken_small/1_walk/3_w.png",
  ];

  CHICKEN_SMALL_DEAD = ["img/3_enemies_chicken/chicken_small/2_dead/dead.png"];

  constructor() {
    super().loadImage("img/3_enemies_chicken/chicken_small/1_walk/1_w.png");
    this.loadImages(this.CHICKEN_SMALL_WALKING);
    this.loadImages(this.CHICKEN_SMALL_DEAD);
    this.x = 400 + Math.random() * 2500;
    this.y = 370;
    this.speed = 0.85 + Math.random() * 2.95;
    this.animate();
  }


  /**
   * Animates the small chicken.
   */
  animate() {
    this.walkingInterval = setInterval(() => {
      this.x -= this.speed;
    }, 1000 / 60); //60x pro Sekunde wird diese Funktion ausgeführt
    this.moveLeft();
    this.walkingAnimationInterval = setInterval(() => {
      this.playAnimation(this.CHICKEN_SMALL_WALKING);
    }, 100);
  }


  /**
   * Handles the small chicken's death animation.
   */
  die() {
    this.stopWalkingAnimation();
    this.playAnimation(this.CHICKEN_SMALL_DEAD);
    this.playChickenDeadSound();
    this.removeChicken();
  }


  /**
   * Stops the walking animation of the small chicken.
   */
  stopWalkingAnimation() {
    clearInterval(this.walkingInterval);
    clearInterval(this.walkingAnimationInterval);
  }


  /**
   * Removes the small chicken from the game.
   */
  removeChicken(){
    setTimeout(() => {
      this.img = new Image();
      this.y = 0;
    }, 80);
  }

  
  /**
   * Plays the sound of the small chicken dying.
   */
  playChickenDeadSound(){
    this.chicken_sound.volume = 0.1;
    this.chicken_sound.play();
  }
}